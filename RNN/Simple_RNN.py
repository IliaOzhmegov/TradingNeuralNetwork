# Data Loader created by Lena, Model Created by Yusif
import pandas as pd
import math
...
df = pd.read_csv("C:/Users/mryus/PycharmProjects/TradingNeuralNetwork/data/sequences/0.csv", index_col=0, parse_dates=True)
df.head(10)
...
model = Sequential()
model.add(Input(shape=(n_timesteps,n_features), dtype="float"))
model.add(layers.SimpleRNN(128))
model.add(Dropout(0.2))
model.add(Dense(20, activation='relu'))
model.add(Dense(20, activation="sigmoid"))
model.summary()                     # getting the summary of our model
